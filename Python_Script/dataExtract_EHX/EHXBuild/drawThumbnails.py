...
class GenPreview():
	...
	def previewMain(self):
		...
		for row in self.panelData:
			...
			if not os.path.exists(f'Python_Script/dataExtract_EHX/output/{jobid[0][0]}/{row[0]}'):
				os.makedirs(f'Python_Script/dataExtract_EHX/output/{jobid[0][0]}/{row[0]}')
			if not os.path.exists(f'Python_Script/dataExtract_EHX/output/{jobid[0][0]}/{row[0]}/Assemblies'):
				os.makedirs(f'Python_Script/dataExtract_EHX/output/{jobid[0][0]}/{row[0]}/Assemblies')


			name = (f'Python_Script/dataExtract_EHX/output/{jobid[0][0]}/{row[0]}/{row[0]}' + '.png')

			self.buildImage(name, row, self.elementData)
			sqlPanel = row[0]
			pgDB.open()
			...
			for assembly in assemblies:
				# Get Assebly Data from Elements
				sql_assembly = f"""select *	from cad2fab.system_elements se 
				...
				name = (f'Python_Script/dataExtract_EHX/output/{jobid[0][0]}/{row[0]}/' + 'Assemblies/' + assName[0][0] +'-' + str(assembly[0]) + '.png')
				subs = []
				for sub in subElements:
					subs.append([sub[0],sub[1],sub[2],sub[17],sub[18],sub[19],
											sub[20],sub[21],sub[22],sub[23],sub[24]])
				
				self.buildImage(name, row, subs)
	...
	def buildImage(self, name, panel, elementData):
			row = panel
			...
			for elemrow in elementData:
				...
				if row[0] == elemrow[0] and ((elemrow[2] == 'Board' or elemrow[2] == 'Sheet') or elemrow[2] == 'Sub-Assembly Board'):
			...
			outimage = Image.new('RGBA',(scaledWidth,1080),(255,255,255,255))
			#Enter draw mode
