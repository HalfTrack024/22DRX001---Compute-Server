...
def check_board_pick(pick_data) -> bool:
    ...
    if not len(pick_data) == 9:  # desired number of keys for a pick operation is 9
        status += 1
        logging.debug("Incorrect Amount of Keys in Board Pick")
        # Check Keys

    if not pick_data.get("missionID") == 400:  # desired number of keys for a pick operation is 9
        status += 1
        logging.debug(f"Incorrect missionID - currently: {pick_data.get('missionID')}")
...
def check_board_place(place_data) -> bool:
    ...
    if not place_data.get("missionID") in [401, 402, 403]:  # desired number of keys for a pick operation is 9
        logging.debug(f"Incorrect missionID - currently: {place_data.get('missionID')}")
        status += 1
    ...
    if place_corner_y < 0:
        status += 1
        logging.debug(f"Info 2 is less than 0")
    if fasten_corner_y < 0:
        status += 1
        logging.debug(f"Info 6 is less than 0")

    if (fasten_corner_y - place_corner_y) > 8:
        status += 1
        logging.debug(f"Y-Positions of data not allowed (Info 2, 6)")

    place_corner_x = place_data.get("Info_01")
    fasten_corner_x = place_data.get("Info_05")
    if place_corner_x < -310:
        status += 1
        logging.debug(f"Info 2 is less than -310 (cannot place that far negative)")
    if fasten_corner_x < 0:
        status += 1
        logging.debug(f"Info 6 is less than 0")
    if (abs(place_corner_x) + fasten_corner_x) > 340:
        status += 1
        logging.debug(f"X-Positions of data not allowed (Info 1, 5)")
