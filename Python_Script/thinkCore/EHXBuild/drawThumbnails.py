...
class GenPreview():
    ...
    def previewMain(self, path):
        ...
        for row in self.panelData:
            ...
            if not os.path.exists(path + f'/{jobid[0][0]}/'):
                os.makedirs(path + f'/{jobid[0][0]}/')
            if not os.path.exists(path + f'/{jobid[0][0]}/{row[0]}'):
                os.makedirs(path + f'/{jobid[0][0]}/{row[0]}')
            if not os.path.exists(path + f'/{jobid[0][0]}/{row[0]}/Assemblies'):
                os.makedirs(path + f'/{jobid[0][0]}/{row[0]}/Assemblies')

            name = (path + f'/{jobid[0][0]}/{row[0]}/{row[0]}' + '.png')

            self.build_image(name, row, self.elementData)
            sqlPanel = row[0]
            pgDB.open()
            ...
            for assembly in assemblies:
                ...
                name = (path + f'/{jobid[0][0]}/{row[0]}/' + 'Assemblies/' + assName[0][0] + '-' + str(
                    assembly[0]) + '.png')
                subs = []
    ...
    def build_image(self, name, panel, element_data):
        ...
        for elem_row in element_data:
            ...
            if row[0] == elem_row[0] and ((elem_row[2] == 'Board' or elem_row[2] == 'Sheet') or elem_row[2] == 'Sub-Assembly Board'):
        ...
        for elem_row in element_data:
            ...
            if elem_row[0] == row[0] and elem_row[2] == 'Sheet':
        ...
        for elem_row in element_data:
            ...
            if elem_row[0] == row[0] and (elem_row[2] == 'Board' or elem_row[2] == 'Sub-Assembly Board'):
                coordinates = [(int(round((elem_row[3] - translateX) * scale)) + 40,
                                int(round((elem_row[4] - translateY) * scale)) + 40),
                               (int(round((elem_row[5] - translateX) * scale)) + 40,
                                int(round((elem_row[6] - translateY) * scale)) + 40),
